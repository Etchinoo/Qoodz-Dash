...
export const CampaignCardV2 = (data, updateActiveCampaignsState) => {
    ...
    data && (
      <RootWrapperCampaignCard key={data.id}>
        <Col gap={"30px"}>
          ...
          <Row spread>
            <StatusCard status={data.status == "active" ? true : false}>
              {/* <IndecatorBlock /> */}
              <Col>
                <StateActive status={data.status == "active" ? true : false}>
                  Status: {data.status == "active" ? "Acitve" : "Paused"}
                </StateActive>
                <Row gap={"13px"}>
